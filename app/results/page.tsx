...
const ResultsPage = () => {
    ...
    if (verificationData) {
      try {
        ...
        toast({
            ...
            action: <ToastAction onClick={handleBackClick} altText="Try again">Try again</ToastAction>,
          })
        setLoading(false)
      }
    }else {
  ...
  const verificationStatusClass = result === 'Passed' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700';
  ...
  return (
    <div className='flex flex-col items-center justify-center min-h-screen bg-gray-100 px-4 md:px-6'>
      {data ? (
          ...
          <div className="w-full max-w-xs">
            <div className="bg-white rounded-lg shadow-md p-6 w-full">
              ...
              <div className="flex items-center justify-around">
                  ...
                  <h2 className="text-lg font-medium text-gray-700 ml-2">Verification Results</h2>
                  <div></div>
                </div>
                <div className="w-full max-w-xs">
                ...
                <div className="w-full max-w-xs">
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300" htmlFor="aadhar">
                    Aadhar Number
                  </label>
                  <input
                    className="block w-full p-2 border border-slate-200 rounded-md focus:ring-indigo-500 focus:border-indigo-500"
                ...
                <div className="w-full max-w-xs">
                <div className={`w-full max-w-xs bg-green-100 rounded-md p-4 ${verificationStatusClass}`}>
                    Verification Status: {result === 'Passed' ? 'Verified' : 'Verification Failed'}
                </div>
                {result !== 'Passed' && (
                 <Button onClick={handleBackClick} className='mt-4 w-full max-w-xs'>
                    Try Again
                 </Button>
                )}
                </div>
        ...
        <p>No verification data found or verification failed.</p>
      )}
    </div>
  );
};
